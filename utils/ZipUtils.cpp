...
string ZipUtils::readZipFile(string zipFile, string fileInZip) {
    ...
    if (uf == NULL) {
//        Log::LogUtils::getLogUtilsInstance()->error("Cannot open %s",zipFile);
        return sout;
    } // file is open
  ...
  //      cerr << "File " << fileInZip << " not found in " << zipFile << endl;
   //     Log::LogUtils::getLogUtilsInstance()->error("File %s not found in %",zipFile);
        return sout;
   ...
   // Log::LogUtils::getLogUtilsInstance()->info("Extracting: {} from zipFile",filename_inzip);
    do {
